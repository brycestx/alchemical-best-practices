...
\item Do I want to run absolute or relative binding calculations
\item What can we do to challenge the paradigm of simulations without appropriate parameters
\item How long should it take to set systems up to make an impact on drug discovery projects
\end{itemize}
\end{checklist}
...
An example of this approach for relative binding free energy calculations includes saving configurations from short 10 picosecond simulations starting at lambda 0, and evaluating changes in the standard error of mean (SEM) in dU/d$\lambda$ as re-evaluated at new values of $\lambda$ starting with a step size of 0.01 up to a predetermined threshold. 
This threshold can be empirically defined as the number of atoms being inserted or deleted in the simulation / 100. 
If the absolute value of the difference in SEM is below the defined threshold then the change in $\lambda$ for re-evaluation of SEM in dU/d$\lambda$ is defined by the distance of the current difference in SEM from the threshold divided by the derivative of SEM with respect to $\lambda$.

When analyzing the results of free energy calculations, convergence is an important component of how reliable an estimate may be. 
If the rate of change in the free energy estimate is large when a calculation is finished, this indicates that the simulation is not locally converged and more sampling is necessary to find an estimate that no longer experiences large change over time.
Therefore it is important use convergence metrics to estimate when a calculation should be stopped.
Examples of stopping condition methods include using standard error to determine when uncertainty in the free energy estimate is sufficient, evaluating the SEM in dU/d$\lambda$ for each $\lambda$, calculating the variance in free energy using bootstrap analysis, and comparing differences in free energies calculated using different percentages of the simulation in both the forward and reverse directions.
...
Additionally, absolute binding calculations often require more sampling due to the complete annihilation of the molecule from its environment [ref JC please advise].
Enhanced sampling techniques have been proposed to try and overcome barriers in free energy calculations and mitigate convergence to the erroneous estimates [ref].
